...
resource "google_service_account" "gemini_sa" {
  ...
  display_name = "Gemini Enterprise Service Account"
}
...
resource "google_storage_bucket_iam_member" "datastore_gcs_viewer" {
  bucket = var.unstructured_data_bucket_name
  ...
  provisioner "local-exec" {
    command = <<-EOT
      gcloud alpha discovery-engine documents import \
        ...
        --gcs-source="gs://cloud-samples-data/gen-app-builder/search/cymbal-bank-employee" \
        --format=document
    EOT
  }
